...
export default defineEventHandler(async (event) => {
  ...
  if (session.user.platformRole !== 'subscriber') {
    throw createError({ statusCode: 403, statusMessage: 'Subscriber portal access only' })
  }
  ...
  if (!membershipOrgIds.length) {
    throw createError({ statusCode: 404, statusMessage: 'No subscribers available for this account' })
  }
  if (requestedOrganizationId && !membershipOrgIds.includes(requestedOrganizationId)) {
    throw createError({ statusCode: 403, statusMessage: 'Subscriber not available in this account' })
  }
  ...
  const tenants = await db
    ...
    .where(inArray(subscribersTable.organizationId, membershipOrgIds))
    .orderBy(desc(subscribersTable.createdAt))
  const tenantById = new Map(tenants.map((subscriber) => [subscriber.id, subscriber]))

  const productIds = products.map((product) => product.id)
  const subscriberIds = tenants.map((subscriber) => subscriber.id)
  ...
  const usageWhere =
    subscriberIds.length && productIds.length
      ...
      : subscriberIds.length
        ? and(inArray(usageRecordsTable.subscriberId, subscriberIds), isNull(usageRecordsTable.productId))
        : null
  ...
  for (const subscriber of tenants) {
    const entry = orgCounts.get(subscriber.organizationId)
    if (entry) {
      entry.subscriberCount += 1
    }
  }

  const tenantOrgById = new Map(tenants.map((subscriber) => [subscriber.id, subscriber.organizationId]))
  ...
  for (const subscription of subscriptions) {
    const orgId = tenantOrgById.get(subscription.subscriberId)
  ...
  for (const license of licenses) {
    const orgId = tenantOrgById.get(license.subscriberId) ?? productOrgById.get(license.productId)
    const entry = orgId ? orgCounts.get(orgId) : null
    if (!entry) {
      continue
    }
  ...
  for (const usageRecord of usageRecords) {
    const orgId = tenantOrgById.get(usageRecord.subscriberId)
    const entry = orgId ? orgCounts.get(orgId) : null
    if (entry) {
  ...
  for (const entitlement of entitlements) {
    const orgId = tenantOrgById.get(entitlement.subscriberId) ?? productOrgById.get(entitlement.productId)
    const entry = orgId ? orgCounts.get(orgId) : null
    if (entry) {
  ...
  const selectedTenantIds = tenants.filter((subscriber) => subscriber.organizationId === activeOrganizationId).map((subscriber) => subscriber.id)
  ...
  const selectedSubscriptions = subscriptions.filter((subscription) => selectedTenantIds.includes(subscription.subscriberId))
  const selectedLicenses = licenses.filter(
    (license) => selectedTenantIds.includes(license.subscriberId) || selectedProductIds.includes(license.productId),
  )
  const selectedUsage = usageRecords.filter((record) => {
    if (!selectedTenantIds.includes(record.subscriberId)) {
      return false
    }
    return record.productId ? selectedProductIds.includes(record.productId) : true
  })
  const selectedEntitlements = entitlements.filter(
    (row) => selectedTenantIds.includes(row.subscriberId) || selectedProductIds.includes(row.productId),
  )
  ...
  const licenseCards = selectedLicenses.map((license) => {
    ...
    return {
      id: license.id,
      subscriberName: tenantById.get(license.subscriberId)?.name ?? license.subscriberId,
  ...
  const subscriptionCards = selectedSubscriptions
    .map((subscription) => {
      ...
      const subscriber = tenantById.get(subscription.subscriberId)
      const item = subscriptionListItem(subscription, plan, product?.name ?? '', product?.id ?? plan?.productId ?? '', subscriber)
      return {
        ...item,
  ...
  const usageCards = selectedUsage.map((usageRecord) => {
    ...
    const subscriber = tenantById.get(usageRecord.subscriberId)
    return {
      id: usageRecord.id,
      subscriberName: subscriber?.name ?? usageRecord.subscriberId,
  ...
  const entitlementCards = selectedEntitlements
    .map((entitlement) => {
      ...
      const subscriber = tenantById.get(entitlement.subscriberId)
      const summary = summarizeEntitlement(entitlement.payloadJson)
      return {
        id: entitlement.id,
        subscriberName: subscriber?.name ?? entitlement.subscriberId,
  ...
  return {
    ...
    overview: {
      ...
      subscriberCount: selectedCounts.subscriberCount,
    ...
    entryPoints: [
      ...
      },
      {
        ...
        description: 'Inspect effective modules and limits in the active subscriber.',
